...
module.exports = {
  async getCrypto(value) {
    ...
    try {
      ...
      let volume = $2('div[class="sc-57oli2-0 dEqHl cmc-body-wrapper"]').find('div[class="sc-16r8icm-0 iTsqJR statsSection___2aZ29"]').find('div[class="statsValue___2iaoZ"]').eq(2).text()
      let marketcap = $2('div[class="sc-1xxpehh-1 dXdBcN"]').find('div[class="sc-16r8icm-0 iTsqJR statsSection___2aZ29"]').find('div[class="statsItemRight___yJ5i-"]').children('div').eq(0).text()
      let change = $('div[class="currency-card relative bg-blue-gray-9 rounded-md p-16"]').find('div[class="flex justify-between mt-8"]').children('div').children('div').children('div').eq(0).text()
      ...
      let usd = $2('div[class="sc-16r8icm-0 hKZfDX container"]').find('div[class="sc-16r8icm-0 fIhwvd"]').children('table').children('tbody').children('tr').eq(0).children('td').text()
